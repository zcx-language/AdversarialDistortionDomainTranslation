...
# @File         : mirflickr_pair_module.py
# @Author       : chengxin
# @Email        : zcx_language@163.com
# @CreateTime   : 2022/3/28 下午9:06
...
from .mirflickr_pair import MirflickrPair


class MirflickrPairModule(LightningDataModule):
    """
    ...
    def setup(self, stage: Optional[str] = None) -> None:
        ...
        if not self.train_data and not self.val_data and not self.test_data:
            self.train_data = MirflickrPair(**self.hparams.dataset_cfg)
            self.val_data = MirflickrPair(**self.hparams.dataset_cfg, stage='val')
            self.test_data = MirflickrPair(**self.hparams.dataset_cfg, stage='test')
